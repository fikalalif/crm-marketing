...
@section('content')
    <div
        ...
        <div class="mb-6 border-b-2 border-black dark:border-white pb-4 transition-colors">
            <h3 class="font-bold text-xl dark:text-white">Sunting Data Akun</h3>
            <p class="text-slate-600 dark:text-slate-400 text-sm">Perbarui informasi anggota tim atau ganti password mereka
                di sini.</p>
        </div>
        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST"
            ...
            <div class="flex flex-col gap-1">
                ...
                <select name="role" id="role" required
                    class="p-2 border-2 border-black dark:border-white focus:outline-none focus:bg-slate-50 dark:focus:bg-slate-600 bg-white dark:bg-slate-700 text-black dark:text-white transition-colors">
                    <option value="marketing" {{ old('role', $user->role) == 'marketing' ? 'selected' : '' }}>Marketing
                    </option>

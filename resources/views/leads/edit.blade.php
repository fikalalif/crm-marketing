...
<div class="max-w-3xl bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6 rounded-sm">
    ...
    <form action="{{ route('leads.update', $lead->id) }}" method="POST" class="flex flex-col gap-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="flex flex-col gap-1">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="close" {{ old('status', $lead->status) == 'close' ? 'selected' : '' }}>Close</option>
                </select>
            </div>
            ...
            <div class="flex flex-col gap-1">
                ...
                <select name="source" id="source"
                    ...
                    <option value="Google Ads" {{ old('source', $lead->source) == 'Google Ads' ? 'selected' : '' }}>Google Ads</option>
                    <option value="Meta Ads" {{ old('source', $lead->source) == 'Meta Ads' ? 'selected' : '' }}>Meta Ads</option>
                    ...
                    <option value="Organic" {{ old('source', $lead->source) == 'Organic' ? 'selected' : '' }}>Organic</option>
                </select>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-1">
            ...
            <textarea name="notes" id="notes" rows="4"
                class="p-2 border-2 border-black focus:outline-none focus:bg-slate-50 transition-colors">{{ old('notes', $lead->notes) }}</textarea>
        </div>
